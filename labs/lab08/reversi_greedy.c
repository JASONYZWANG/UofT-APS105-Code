...
#include <stdio.h>
#include <stdbool.h>
// Forward declarations of your existing helper functions
...
// New helper functions for Lab 8
...
int main(void) {
    ...
    if (n<4 || n>26 || n%2) { fputs("Use an even dimension from 4 to 26.\n", stderr); return 1; }
    char comp_color;
    printf("Computer plays (B/W): ");
    if (scanf(" %c", &comp_color) != 1) { fputs("Invalid or incomplete input.\n", stderr); exit(EXIT_FAILURE); }
    if (comp_color!='B' && comp_color!='W') { fputs("Choose B or W.\n", stderr); return 1; }
    ...
    while (!game_over) {
        ...
        if (current_turn == comp_color) {
            ...
            if (scanf(" %c%c", &charrow, &charcol) != 2) { fputs("Invalid or incomplete input.\n", stderr); exit(EXIT_FAILURE); }
