...
int main(void){
    ...
    do{
        ...
        if (scanf("%lf",&d) != 1) { fputs("Invalid or incomplete input.\n", stderr); exit(EXIT_FAILURE); }
    }while(d<3 || d>30);
    do{
        printf("Please enter the target height between 3 and 6 m:\n");
        if (scanf("%lf",&H) != 1) { fputs("Invalid or incomplete input.\n", stderr); exit(EXIT_FAILURE); }
    }while(H<3 || H>6);
    ...
    for(double i=0.00;i<=90.00;i+=1.00){
        ...
        time=d/speed_x; //time of fly
        height=L+speed_y*time+(0.5)*(-g)*(time*time); //height in the end
