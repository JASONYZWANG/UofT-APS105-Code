...
bool checkLegalInDirection(char board[][26],int n,int row,int col,char colour,int dr,int dc);
#include <stdlib.h>
#include <stdio.h>
...
int main(void){
    ...
    int real_size;
    if (scanf("%d", &real_size)!=1 || real_size<4 || real_size>26 || real_size%2) { fputs("Use an even dimension from 4 to 26.\n", stderr); return 1; }
...
void Initialize_Board_Configuration(char(*board)[26],int n){
    ...
    for(;true;){
        ...
        if (scanf(" %c %c %c",&color, &row_index, &col_index) != 3) { fputs("Invalid or incomplete input.\n", stderr); exit(EXIT_FAILURE); }
        if(color=='!' && row_index=='!' && col_index=='!') {
            return;
        }
        else if((color=='B' || color=='W' || color=='U') && positionInBounds(n,(int)row_index-97,(int)col_index-97)==true){
...
void Make_One_Move(char(*board)[26],int n){
    ...
    printf("Enter a move:\n");
    if (scanf(" %c %c %c",&Colour,&Arow,&Acol) != 3) { fputs("Invalid or incomplete input.\n", stderr); exit(EXIT_FAILURE); }
    int row=(int)(Arow-'a'),col=(int)(Acol-'a');
    if((Colour!='B' && Colour!='W') || positionInBounds(n, row, col)==false){//Out of boundary
        printf("Invalid move.\n");
        printBoard(board,n);
        return;
    }
