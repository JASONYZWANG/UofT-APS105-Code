...
int main(void){
    ...
    //initialization
    printf("Enter input: ");
    if (scanf("%d%d%d",&alive_cell,&rule_num,&iterate) != 3) { fputs("Invalid or incomplete input.\n", stderr); exit(EXIT_FAILURE); }
    if (alive_cell < 0 || alive_cell >= ARR_SIZE || rule_num < 0 || rule_num > 255 || iterate < 0 || iterate > 10000) { fputs("Invalid automaton parameters.\n", stderr); return 1; }
