...
//2.0
//calculate the help score and weight for a position
// int getPosWeight(int r, int c, int n) {
//     //corner will get more score
//     if ((r == 0 || r == n - 1) && (c == 0 || c == n - 1)) return 1000;
//     // 2. X-Square
//     if ((r == 1 || r == n - 2) && (c == 1 || c == n - 2)) return -200;
//     // 3. C-Square 
//     if (((r == 0 || r == n - 1) && (c == 1 || c == n - 2)) ||
//         ((r == 1 || r == n - 2) && (c == 0 || c == n - 1))) return -100;
//     // Edge
//     if (r == 0 || r == n - 1 || c == 0 || c == n - 1) return 50;
//     return 1;
// }
...
//     for (int i = 0; i < n; i++) {
//         for (int j = 0; j < n; j++) {
//             if (board[i][j] == 'U' && Judge_Available_Move((char(*)[26])board, n, turn, i, j)) {//check is this position is legal
...
//                 int dC[] = {-1, 0, 1, -1, 1, -1, 0, 1};
//                 for (int d = 0; d < 8; d++) {
//                     flips += Count_Flips_InDirection((char(*)[26])board, n, i, j, turn, dR[d], dC[d]);
//                 }
//                 //score 
//                 double current_score = getPosWeight(i, j, n) + (flips * 1.5);

//                 //simulation and predict for this step
//                 char tempBoard[26][26];
//                 for(int m=0;m<26;m++){
//                     for(n=0;n<26;n++){
//                         tempBoard[m][n]=board[m][n];
//                     }
//                 }//make a same draft board;
                
//                 tempBoard[i][j] = turn;
//                 Execute_Flips(tempBoard, n, i, j, turn); //execute this flip to see whether this would give opponent chance.

//                 bool gives_opponent_corner = false;
//                 if (Judge_Available_Move(tempBoard, n, opponent, 0, 0) ||
//                     Judge_Available_Move(tempBoard, n, opponent, 0, n-1) ||
//                     Judge_Available_Move(tempBoard, n, opponent, n-1, 0) ||
//                     Judge_Available_Move(tempBoard, n, opponent, n-1, n-1)) {
//                     gives_opponent_corner = true;
//                 }

//                 if (gives_opponent_corner) {
//                     current_score -= 600; //if this position will give opponent chance
//                 }
